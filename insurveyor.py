...
from insurveyor_random_pos_generator import RandomPositionGenerator
import numpy as np
...
start = time.time()
read_categorizer_cmd = SURV_PATH + "/bin/insurveyor_reads_categorizer %s %s %s" % (cmd_args.bam_file, cmd_args.workdir, cmd_args.reference)
...
start = time.time()
clip_consensus_builder_cmd = SURV_PATH + "/bin/insurveyor_clip_consensus_builder %s %s" % (cmd_args.workdir, cmd_args.reference)
...
start = time.time()
call_insertions_cmd = SURV_PATH + "/bin/insurveyor_call_insertions %s %s %s" % (cmd_args.workdir, cmd_args.reference, sample_name)
...
start = time.time()
dc_remapper_cmd = SURV_PATH + "/bin/insurveyor_dc_remapper %s %s %s" % (cmd_args.workdir, cmd_args.reference, sample_name)
...
start = time.time()
add_filtering_info_cmd = SURV_PATH + "/bin/insurveyor_add_filtering_info %s %s %s" % (cmd_args.bam_file, cmd_args.workdir, cmd_args.reference)
print("Executing:", add_filtering_info_cmd)
os.system(add_filtering_info_cmd)
end = time.time()
print("Filtering info added in %d [s]" % (end-start))

filter_cmd = SURV_PATH + "/bin/insurveyor_filter %s %s 0.25" % (cmd_args.workdir, cmd_args.reference)
